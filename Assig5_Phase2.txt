...
public class Assig5_Phase2 {
    ...
    static final int NUM_CARD_IMAGES = 57; // 52 + 4 jokers + 1
    ...
    // Phase 2 Main: main to throw all the JLabels
...
class CardTable extends JFrame {
    ...
    // Changed private int numPlayers to public static int numPlayers
...
class Card {
    ...
    public String toString() {
        // returns value and suit of card or a string if errorFlag true
        if (errorFlag) {
            return "[invalid]";
        } else {
     ...
     *
     * @param: Takes a character for value and a Suit data type as suit
...
class Hand {
    ...
    /**
     * takeCard: Adds card to next available position in myCards array
     * (your hand) if there's space.
     *
     * @param card The input card object to be taken
     * @return returns true if card taken from deck, false if hand full
     */
    ...
    /**
     * playCard: returns and removes card in top of the array
     *
    ...
    public Card playCard(int cardIndex) {
        ...
        if (numCards == 0) {
            ...
            // Otherwise we "play" the card & adjust the rest of the hand
     ...
     *
     * @param: takes integer and checks card in the index of integer k
...
class Deck {
    ...
    public Deck(int numPacks) {
        ...
        if (numPacks > 0 && numPacks < 7) {
            this.cards = new Card[DECK_SIZE * numPacks]; // increase size
            this.init(numPacks);
    ...
    /**
     * shuffle: Mixes up cards by using the standard random generator to
     * procedurally swap card w/ a random other. Does this toShuffleCount
    ...
    public void shuffle() {
        ...
        for (int j = 0; j < toShuffleCnt; j++) {
            for (int i = 0; i < topCard; i++) {
                int change = shuffleCards.nextInt(topCard); // Index
    ...
    /**
     * allocateMasterPack: Constructs a playing card deck to be copied
     * Even if many Deck objects are constructed, this will not
    ...
    /**
     * addCard: Check if there are not too many instances of the card in
    ...
    /**
     * removeCard: Removes a card from the deck. Put the current top
     * card into its place. Checks the card you need is actually still in
     * the deck first.
     *
    ...
    public boolean removeCard(Card card) {
        ...
        for (int i = 0; i < cards.length; i++) {
            if (cards[i].equals(card)) {
                // If the card in the deck is EQUAL to our search card
                toSwapIndex = i;
            }
        }
        ...
        if (doSwap) {
            ...
            Card temp = new Card(cards[toSwapIndex]); // Our Mid-Card
    ...
    /**
     * sort(): put all of the cards in the deck back into order
     * according to their values. Uses arraySort (bubble sort) to do so.
     */
    void sort() {
